...
class Query(ObjectType):
    products = graphene.List(ProductType,search=graphene.String(),searchtype=graphene.String())
    ...
    def resolve_products(self,info,search=None,searchtype=None,**kwargs):
        if search and searchtype==None:
            ...
            filter=(
                Q(productCategory__icontains=search)
            )
            return Product.objects.filter(filter)
            ...
            filter=(
                Q(productName__icontains=search) | Q(productAbout__icontains=search) 
            )
            return Product.objects.filter(filter)
        else :
...
class AddProduct(graphene.Mutation):
    ...
    def mutate(self,info,name:str,ing:str,category:str,price:float,about:str,imageurl:str):
        # Add predict method here
        
        envscored=3.141562789 #here
        prod=Product(productName=name,productIngredients=ing,productCategory=category,productPrice=price,productAbout=about,imageUrl=imageurl,envScore=envscored)
        prod.save()
